...
/** 当前选中的店铺ID，存localStorage持久化 */
export const shopKey = "current-shop-id";
export function getCurrentShopId(): number | null {
  const id = storageLocal().getItem(shopKey);
  return id ? Number(id) : null;
}
export function setCurrentShopId(id: number | null) {
  if (id) storageLocal().setItem(shopKey, id);
  else storageLocal().removeItem(shopKey);
}
