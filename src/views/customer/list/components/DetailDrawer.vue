...
const loadPurchases = async () => {
  if (!props.customerId) return;
  const r = await getCustomerPurchases(props.customerId, purchasesPage.value, pageSize);
  if (r?.success) {
...
const loadPoints = async () => {
  if (!props.customerId) return;
  const r = await getCustomerPoints(props.customerId, pointsPage.value, pageSize);
  if (r?.success) {
...
const adjPoints = () => {
  import("element-plus").then((m) =>
    m.ElMessageBox.prompt("输入积分数量（正数增加，负数扣减）", "积分调整", {
      inputType: "number"
    }).then(async ({ value }: any) => {
...
const walletTypeMap: Record<number, string> = { 1: "充值", 2: "消费", 3: "退款", 4: "调整" };
const purchaseStatusMap: Record<number, string> = { 1: "有效", 2: "已退款", 3: "已过期" };
</script>
...
<template>
  ...
  >
    <template v-if="customer">
      ...
      <div class="mb-4 flex items-start gap-4">
        ...
        <div class="flex-1">
          <div class="flex items-center gap-2 mb-1">
            <span class="text-lg font-semibold">{{ customer.nickname || "-" }}</span>
            <el-tag v-if="customer.wechat_openid" size="small" type="success">微信</el-tag>
          </div>
      ...
      <!-- 钱包卡片 -->
      <div class="rounded-lg border border-[var(--el-border-color-light)] p-4 mb-4">
        <div class="text-sm font-medium mb-3">钱包</div>
        <div class="grid grid-cols-3 gap-4 mb-4">
          <div class="text-center">
            <div class="text-xs text-[var(--el-text-color-secondary)] mb-1">余额</div>
            <div class="text-2xl font-bold" style="color: var(--el-color-primary)">
              ¥{{ wallet?.balance || 0 }}
            </div>
          </div>
          <div class="text-center">
            <div class="text-xs text-[var(--el-text-color-secondary)] mb-1">累计充值</div>
            <div class="text-lg font-medium">¥{{ wallet?.total_recharged || 0 }}</div>
          </div>
          <div class="text-center">
            <div class="text-xs text-[var(--el-text-color-secondary)] mb-1">累计消费</div>
            <div class="text-lg font-medium">¥{{ wallet?.total_spent || 0 }}</div>
          </div>
        </div>
        <div class="flex gap-2">
          <el-button size="small" type="primary" @click="adjWallet(1)">充值</el-button>
          <el-button size="small" type="warning" plain @click="adjWallet(4)">扣减</el-button>
        </div>
      </div>
      ...
      <el-tabs v-model="activeTab">
        ...
        <el-tab-pane label="钱包流水" name="transactions">
          <el-table v-if="transactions.length" :data="transactions" size="small">
            <el-table-column label="类型" width="70">
              <template #default="{ row }">
                <el-tag
                  :type="row.type === 1 ? 'success' : row.type === 3 ? 'warning' : 'info'"
                  size="small"
                >
            ...
            <el-table-column label="金额" width="100">
              <template #default="{ row }">
                <span :style="{ color: row.type === 1 || row.type === 3 ? 'var(--el-color-success)' : 'var(--el-color-danger)' }">
                  {{ row.type === 1 || row.type === 3 ? "+" : "-" }}¥{{ row.amount }}
                </span>
              </template>
            </el-table-column>
        ...
        <el-tab-pane label="积分记录" name="points">
          <div class="mb-3 flex items-center gap-2">
            <el-button size="small" type="primary" @click="adjPoints">积分调整</el-button>
          </div>
          <el-table v-if="pointsList.length" :data="pointsList" size="small">
            <el-table-column prop="points" label="积分变动" width="90">
              <template #default="{ row }">
                <span :style="{ color: row.points > 0 ? 'var(--el-color-success)' : 'var(--el-color-danger)' }">
