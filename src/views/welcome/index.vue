...
const seatTag = computed(() => {
  ...
  if (r <= 7) return "warning";
  return "success";
});
const chanLabel = (v: string) =>
  (
    ({
      store: "门店",
      meituan: "美团",
...
const pkgTypeLabel = (v: number) =>
  (({ 1: "单次", 2: "周卡", 3: "月卡" }) as any)[v] || v;
      ...
      <el-tabs v-model="activeTab" class="dash-tabs">
        ...
        <el-tab-pane label="总览" name="overview">
          ...
          <div class="cards-row">
            ...
            <StatCard
              label="今日核销"
              :value="d.todayCheckins"
              color="#4facfe"
              :loading="loading"
            />
            <StatCard
              label="新顾客"
              :value="d.todayNewCustomers"
              color="#ff9800"
              :loading="loading"
            />
          </div>
          ...
          <div class="mini-row">
            ...
            <div class="mini-item">
              ...
              ><span class="mini-val red">{{ fmtMoney(d.monthExpense) }}</span>
            </div>
            <div class="mini-item">
              <span class="mini-label">本月核销</span
              ><span class="mini-val">{{ d.monthCheckins ?? "-" }} 次</span>
            </div>
          </div>
          <!-- 待处理 + 预警 双栏 -->
          <div class="overview-bottom">
            <div class="overview-left">
              ...
              <div class="pending-grid">
                <div
                  class="pending-card"
                  :class="{ alert: d.pendingRefunds }"
                  @click="go('/trade/refund')"
                >
                  <div
                    class="pending-num"
                    :style="{
                      color: d.pendingRefunds
                        ? 'var(--el-color-danger)'
                        : '#999'
                    }"
                  >
                ...
                <div
                  class="pending-card"
                  :class="{ alert: d.pendingFeedbacks }"
                  @click="go('/feedbacks')"
                >
                  <div
                    class="pending-num"
                    :style="{
                      color: d.pendingFeedbacks
                        ? 'var(--el-color-warning)'
                        : '#999'
                    }"
                  >
                    {{ d.pendingFeedbacks ?? 0 }}
                  </div>
                  <div class="pending-label">待回复评价</div>
                </div>
                <div class="pending-card" @click="go('/trade/checkin')">
                  <div class="pending-num" style="color: #4facfe">
                    {{ d.activeSessions ?? 0 }}
                  </div>
                  <div class="pending-label">进行中游玩</div>
                </div>
                <div class="pending-card">
                  <div class="pending-num" style="color: #999">
                    {{ d.todayQueue ?? 0 }}
                  </div>
            ...
            <div class="overview-right">
              ...
              <div class="section-title">
                库存预警
                <el-button
                  v-if="(d.warnings?.length || 0) > 0"
                  link
                  type="primary"
                  size="small"
                  @click="go('/inventory/list')"
                  >查看全部</el-button
                >
              </div>
              <div v-if="d.warnings?.length" class="warn-list">
                <div v-for="(w, i) in d.warnings" :key="i" class="warn-item">
                  <span class="warn-name">{{ w.material_name }}</span>
                  <span class="warn-stock"
                    >{{ w.quantity }} / 预警 {{ w.min_stock }}</span
                  >
                </div>
              </div>
        ...
        <el-tab-pane label="营收分析" name="revenue">
          <div class="trend-box">
            <div class="chart-label">近 7 日收入支出</div>
            <ShopRevenueTrend
              :revenues="d.trendRevenues || []"
              :expenses="d.trendExpenses || []"
            />
          </div>
          <div class="mini-row">
            <div class="mini-item">
              <span class="mini-label">30天客单价</span
              ><span class="mini-val green">{{
                fmtMoney(d.avgCustomerPrice)
              }}</span>
            </div>
            <div class="mini-item">
              <span class="mini-label">30天退款率</span
              ><span
                class="mini-val"
                :class="{ red: (d.refundRate || 0) > 0.05 }"
                >{{ fmtPct(d.refundRate) }}</span
              >
            </div>
            <div class="mini-item">
              <span class="mini-label">本月收入</span
                ...
                <div
                  ...
                  :style="{
                    width:
                      Math.min(
                        100,
                        (ch.cnt / (d.channels?.[0]?.cnt || 1)) * 100
                      ) + '%'
                  }"
                />
              </div>
              <span class="chan-info"
                >{{ ch.cnt }} 单 / {{ fmtMoney(ch.amount) }}</span
              >
            </div>
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane label="顾客分析" name="customer">
          <div class="cards-row cards-4">
            ...
            <StatCard
              label="回头率"
              :value="fmtPct(d.returnRate)"
              color="#f5576c"
              :loading="loading"
            />
            <StatCard
              label="储值余额"
              :value="fmtMoney(d.walletBalance)"
              color="#ff9800"
              :loading="loading"
            />
          </div>
        ...
        <el-tab-pane label="运营分析" name="ops">
          ...
          <div v-if="d.topPackages?.length" class="channel-bars">
            <div v-for="(p, i) in d.topPackages" :key="i" class="chan-row">
              ...
              <div class="chan-bar-bg">
                <div
                  class="chan-bar-fill ops-bar"
                  :style="{
                    width:
                      Math.min(
                        100,
                        (p.cnt / (d.topPackages?.[0]?.cnt || 1)) * 100
                      ) + '%'
                  }"
                />
              </div>
          ...
          <div class="mini-row" style="margin-top: 16px">
            <div class="mini-item">
              <span class="mini-label">进行中游玩</span
              ><span class="mini-val" style="color: #4facfe">{{
                d.activeSessions ?? 0
              }}</span>
            </div>
            <div class="mini-item">
              <span class="mini-label">今日排队</span
              ><span class="mini-val">{{ d.todayQueue ?? 0 }}</span>
            </div>
            <div class="mini-item">
              <span class="mini-label">本月核销</span
              ><span class="mini-val">{{ d.monthCheckins ?? 0 }} 次</span>
            </div>
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane label="员工绩效" name="staff">
          <div class="mini-row">
            <div class="mini-item">
              <span class="mini-label">今日打卡</span
              ><span class="mini-val">{{ d.todayAttendCount ?? 0 }} 人</span>
            </div>
            <div class="mini-item">
              <span class="mini-label">今日核销</span
              ><span class="mini-val">{{ d.todayCheckins ?? 0 }} 次</span>
            </div>
          </div>
          <div class="section-title">今日核销排行</div>
          <div v-if="d.staffCheckins?.length" class="channel-bars">
            <div v-for="(s, i) in d.staffCheckins" :key="i" class="chan-row">
              <span class="chan-name">{{ s.name }}</span>
              <div class="chan-bar-bg">
                <div
                  class="chan-bar-fill staff-bar"
                  :style="{
                    width:
                      Math.min(
                        100,
                        (s.cnt / (d.staffCheckins?.[0]?.cnt || 1)) * 100
                      ) + '%'
                  }"
                />
              </div>
...
.chan-bar-fill.ops-bar {
  background: #43e97b;
}

.chan-bar-fill.staff-bar {
  background: #4facfe;
}
