...
const sourceLabel = (val: string) => {
  const map: Record<string, string> = { wallet: "储值钱包", wechat: "微信", alipay: "支付宝", cash: "现金", other: "其他" };
  return map[val] || val || "-";
};
    ...
    <el-row :gutter="16" class="summary-row">
      <el-col :span="8">
        <el-card shadow="never">
          ...
          <div class="text-2xl font-bold" style="color:#667eea">¥{{ summary.totalRevenue ?? 0 }}</div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="never">
          <div class="text-sm text-dim">记录数</div>
          <div class="text-2xl font-bold" style="color:#f5576c">{{ summary.totalCount ?? 0 }}</div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="never">
          <div class="text-sm text-dim">笔均收入</div>
          <div class="text-2xl font-bold" style="color:#4facfe">¥{{ (summary.avgAmount ?? 0).toFixed(2) }}</div>
        </el-card>
      </el-col>
    </el-row>
      ...
      <el-table v-loading="loading" :data="tableData" stripe style="width:100%">
        ...
        <el-table-column label="金额" width="110" align="center">
          <template #default="{ row }">
            <span :style="{ color: Number(row.amount) < 0 ? 'var(--el-color-danger)' : '' }">
              ¥{{ row.amount }}
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="game_session_id" label="场次ID" width="90" />
