...
const loadRules = async () => {
  ...
  try {
    const r = await getCommissionRules({ page: rulesPage.value, size: rulesSize.value });
    if (r?.success) { rules.value = r.data.list; rulesTotal.value = r.data.total; }
...
const loadSettlements = async () => {
  ...
  try {
    const r = await getCommissionSettlements({ page: settlementsPage.value, size: settlementsSize.value });
    if (r?.success) { settlements.value = r.data.list; settlementsTotal.value = r.data.total; }
  } finally { settlementsLoading.value = false; }
};

const openAddRule = () => {
  isEditRule.value = false;
  Object.assign(ruleForm, { ruleId: 0, roleId: "", ruleType: 1, value: "", description: "" });
...
const openEditRule = (row: any) => {
  ...
  Object.assign(ruleForm, {
    ruleId: row.id, roleId: String(row.role_id || ""),
    ruleType: row.rule_type, value: String(row.value || ""),
...
const saveRule = async () => {
  const data: any = { ruleId: ruleForm.ruleId, ruleType: ruleForm.ruleType, value: ruleForm.value, description: ruleForm.description };
  const r = isEditRule.value
    ? await updateCommissionRule(data)
    : await addCommissionRule({ roleId: ruleForm.roleId, ...data } as any);
  if (r?.success) {
    message(isEditRule.value ? "规则已更新" : "规则已添加", { type: "success" });
...
const toggleRule = async (row: any) => {
  ...
  if (r?.success) { message(newActive ? "已启用" : "已禁用", { type: "success" }); loadRules(); }
...
const onGenerateSettlement = async () => {
  ...
  if (value) {
    ...
    if (r?.success) { message("结算已生成", { type: "success" }); loadSettlements(); }
    else message(r?.msg || "失败", { type: "warning" });
  }
};

const onPaySettlement = async (id: number) => {
  try { await ElMessageBox.confirm("确认发放提成？", "提示", { type: "warning" }); }
  catch { return; }
  const r = await paySettlement({ settlementId: id });
  if (r?.success) { message("提成已发放", { type: "success" }); loadSettlements(); }
  else message(r?.msg || "失败", { type: "warning" });
};

const onRuleSizeChange = (s: number) => { rulesSize.value = s; rulesPage.value = 1; loadRules(); };
const onSettlementSizeChange = (s: number) => { settlementsSize.value = s; settlementsPage.value = 1; loadSettlements(); };

onMounted(() => { loadDicts(); loadRules(); loadSettlements(); });
</script>
  ...
  <div class="page-container">
    <el-tabs v-model="activeTab" class="tab-layout">
      <el-tab-pane label="提成规则" name="rules">
          ...
          <el-table v-loading="rulesLoading" :data="rules" style="width:100%">
            ...
            <el-table-column label="操作" width="150" fixed="right">
              <template #default="{ row }">
                ...
                <el-button link :type="row.is_active ? 'warning' : 'success'" size="small" @click="toggleRule(row)">
                  {{ row.is_active ? "禁用" : "启用" }}
                </el-button>
              </template>
            </el-table-column>
        ...
        <el-pagination
          v-model:current-page="rulesPage" v-model:page-size="rulesSize" :total="rulesTotal"
          :page-sizes="[10, 20, 50, 100]" layout="total, sizes, prev, pager, next, jumper"
          class="page-pagination" @size-change="onRuleSizeChange" @current-change="loadRules"
        />
      </el-tab-pane>

      <el-tab-pane label="结算记录" name="settlements">
        <el-button class="mb-2" @click="onGenerateSettlement">生成结算</el-button>
        <div class="page-table">
          <el-table v-loading="settlementsLoading" :data="settlements" style="width:100%">
            <el-table-column prop="staff_name" label="员工" width="100" />
            <el-table-column prop="settlement_period" label="周期" width="90" />
            ...
            <el-table-column label="操作" width="80" fixed="right">
              <template #default="{ row }">
                <el-button v-if="row.status === 1" link type="primary" @click="onPaySettlement(row.id)">发放</el-button>
              </template>
            </el-table-column>
        ...
        <el-pagination
          v-model:current-page="settlementsPage" v-model:page-size="settlementsSize" :total="settlementsTotal"
          :page-sizes="[10, 20, 50, 100]" layout="total, sizes, prev, pager, next, jumper"
          class="page-pagination" @size-change="onSettlementSizeChange" @current-change="loadSettlements"
        />
      </el-tab-pane>
    </el-tabs>
    ...
    <el-dialog v-model="ruleDialog" :title="isEditRule ? '编辑提成规则' : '新增提成规则'" width="480px" :close-on-click-modal="false">
      <el-form :model="ruleForm" label-width="100px">
        ...
        <el-form-item label="值" required>
          <el-input v-model="ruleForm.value" :placeholder="ruleForm.ruleType === 1 ? '每场提成金额' : ruleForm.ruleType === 2 ? '流水比例(%)' : '固定金额'" />
        </el-form-item>
