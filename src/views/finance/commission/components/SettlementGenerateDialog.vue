...
const confirm = async () => {
  if (!period.value) { message("请输入结算周期", { type: "warning" }); return; }
  loading.value = true;
  try {
...
<template>
  <el-dialog
    :model-value="visible" title="生成结算" width="400px"
  ...
  >
    <el-form label-width="80px">
      <el-form-item label="结算周期" required>
        <el-date-picker
          ...
          placeholder="选择结算月份" style="width:100%"
        />
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="loading" @click="confirm">生成</el-button>
    </template>
  </el-dialog>
</template>
