...
const couponChannels = computed(() =>
  channelOptions.value.filter((c: any) =>
    ["meituan", "douyin"].includes(c.dict_label)
  )
);
const walletInfo = ref<{ balance: number; total_recharged: number } | null>(
  null
);
...
const loadDicts = async () => {
  try {
    ...
    if (pmRes?.success && Array.isArray(pmRes.data))
      paymentMethodOptions.value = pmRes.data;
    else
...
const handleSave = async () => {
  ...
  if (
    form.paymentType === "wallet" &&
    walletInfo.value &&
    walletInfo.value.balance < form.totalAmount - couponDiscount.value
  ) {
...
<template>
  ...
  >
    <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item
        v-if="form.customersId && form.packageId && availableCoupons.length > 0"
        label="优惠券"
      >
        <el-select
          ...
          <el-option
            ...
            :label="
              ...
              (cu.type === 1
                ? cu.value
                : cu.type === 2
                  ? ((form.totalAmount * Number(cu.value)) / 100).toFixed(2)
      ...
      <template v-if="form.paymentType === 'wallet' && form.customersId">
        <el-form-item label="钱包余额">
          ...
          <span
            v-else-if="walletInfo"
            :style="{
              color:
                walletInfo.balance < form.totalAmount ? '#f56c6c' : '#67c23a',
              fontWeight: 700
            }"
          >
      ...
      <template v-if="form.paymentType === 'coupon'">
        ...
        <el-form-item label="第三方券码" prop="thirdPartyCouponCode">
          <el-input
            v-model="form.thirdPartyCouponCode"
            placeholder="输入美团/抖音券码"
          />
        </el-form-item>
