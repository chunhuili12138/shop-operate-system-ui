...
const reject=async(id:number)=>{const{v}=await import("element-plus").then(m=>m.ElMessageBox.prompt("输入拒绝原因","拒绝退款"));if(v){const r:any=await http.put("/purchasesRefundsReject",{data:{refundId:id,reason:v}});r?.success?(message("已拒绝",{type:"success"}),load()):message(r?.msg||"失败",{type:"warning"})}};
onMounted(load);
</script>
<template>
