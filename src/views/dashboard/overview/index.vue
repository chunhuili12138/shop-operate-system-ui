...
const statCards = computed(() =>
  isSuper.value
    ? [
        ...
        { label: "本月新增商户", key: "newTenantsThisMonth", color: "var(--color-stat-red)" }
      ]
    : [
        { label: "今日收现", key: "todaySales", color: "var(--color-stat-blue)", prefix: "¥" },
        { label: "今日确认收入", key: "todayRevenue", color: "var(--color-stat-green)", prefix: "¥" },
        { label: "今日订单", key: "todayOrders", color: "var(--color-stat-orange)" },
        { label: "今日核销", key: "todayCheckins", color: "var(--color-stat-purple)" },
        { label: "新顾客数", key: "todayNewCustomers", color: "var(--color-stat-red)" }
      ]
);
...
const load = async () => {
  ...
  try {
    ...
    if (!isSuper.value) {
      const w = await getInventoryWarnings();
      if (w?.success && Array.isArray(w.data)) warnings.value = (w.data as any[]).slice(0, 5);
    }
  } finally { loading.value = false; }
};

const goInventory = () => router.push("/inventory/list");
...
<template>
  <div class="app-container">
    <el-row :gutter="16" class="mb-4">
      <el-col v-for="card in statCards" :key="card.key" :lg="isSuper ? 6 : 4.8" :md="8" :sm="12" :xs="24">
    ...
    <el-row :gutter="16">
      <el-col :span="16">
        <el-card shadow="never">
          <template #header><span class="font-medium">到期提醒</span></template>
          <div v-if="data.expiringSoon?.length">
            <div v-for="item in data.expiringSoon" :key="item.id"
        ...
        <el-card shadow="never">
          <template #header>
            <div class="flex justify-between items-center">
              ...
              <el-button v-if="warnings.length" link type="primary" size="small" @click="goInventory">查看全部</el-button>
            </div>
          </template>
          <div v-if="warnings.length">
            <div v-for="(w, i) in warnings" :key="i"
              class="text-sm py-1 border-b border-[var(--ad-border)] last:border-0"
              style="color: var(--el-color-danger)">
              {{ w.material_name || w.name }} — 库存 {{ w.quantity }} / 预警 {{ w.min_stock }}
            </div>
          </div>
          <div v-else class="text-dim">暂无库存预警</div>
        </el-card>
      </el-col>
    </el-row>
  </div>
