...
const statusType = (s: number): any => ({ 1: "warning", 2: "success", 3: "info" } as any)[s] || "info";
...
const openReply = (row: any) => { currentRow.value = row; replyVisible.value = true; };

const loadDicts = async () => {
  try {
    const [t, s] = await Promise.all([getDictData("feedback_type"), getDictData("feedback_status")]);
    if (t?.success && Array.isArray(t.data)) typeDict.value = t.data;
    if (s?.success && Array.isArray(s.data)) statusDict.value = s.data;
  } catch { /* ignore */ }
};

const onSizeChange = (v: number) => { size.value = v; page.value = 1; load(); };
...
<template>
  <div class="page-container">
    <div class="page-header">
      <div class="page-header-actions"><div /><div><el-button type="primary" @click="load">刷新</el-button></div></div>
    </div>
    <div class="page-table">
      <el-table v-loading="loading" :data="tableData" stripe style="width:100%">
        <el-table-column prop="customer_name" label="顾客" width="100" />
        <el-table-column label="类型" width="80">
          <template #default="{ row }">{{ typeLabel(row.feedback_type) }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="80" fixed="right">
          <template #default="{ row }">
            <el-button v-if="row.status === 1" link type="primary" size="small" @click="openReply(row)">回复</el-button>
    ...
    />
    <ReplyDialog v-model:visible="replyVisible" :data="currentRow" @success="load" />
  </div>
</template>
