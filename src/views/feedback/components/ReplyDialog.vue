...
<template>
  ...
  >
    <el-descriptions v-if="data" :column="1" border size="small">
      ...
      >
      <el-descriptions-item v-if="data.reply_content" label="已回复">{{
        data.reply_content
      }}</el-descriptions-item>
    </el-descriptions>
    <el-form
    ...
    <template #footer>
      ...
      <el-button v-if="!(data?.status === 2)" type="primary" @click="save"
        >提交回复</el-button
      >
    </template>
  </el-dialog>
